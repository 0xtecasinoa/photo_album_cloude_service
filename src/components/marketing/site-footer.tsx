...
const COLUMNS = [
  {
    title: 'プロダクト',
    links: ['らくらくカメラ', 'らくらくデスクトップ', 'らくらくクラウド', '機能一覧', '動作環境', 'アップデート情報'],
  },
  {
    title: 'サポート',
    links: ['ヘルプセンター', 'よくあるご質問', 'お問い合わせ', '導入サポート', 'セミナー・動画', 'お知らせ'],
  },
];
        ...
        <div className="grid gap-12 lg:grid-cols-[1.3fr_1fr_1fr_1.1fr]">
          <div>
            ...
            <div className="mt-8 flex items-center gap-3 rounded-[10px] bg-white/10 px-4 py-3">
              <ShieldCheck className="size-5 shrink-0" strokeWidth={1.8} aria-hidden />
          ...
          {COLUMNS.map((col) => (
            <div key={col.title}>
              ...
              <ul className="mt-5 space-y-3">
                {col.links.map((l) => (
                  <li key={l}>
                    <Link href="#" className="text-[13px] text-white/85 hover:text-white hover:underline">
                      {l}
                    </Link>
                  </li>
                ))}
          ...
          <div>
            ...
            <p className="mt-5 text-[13px] text-white/85">ご相談・ご質問はこちらから</p>
            <Link
              href="/contact"
